    ...
    return (
        <div className={`flex flex-col items-start ${isEven ? 'text-brand border-brand' : 'text-white border-white'} border-b-8`}>
            <div className="w-full flex justify-between items-center ">
                <p className="font-display text-2xl text-center ">{format(gig.date, 'dd/MM - HH:mm')}</p>
                <p className="font-display text-2xl  text-center ">{FORMATS[gig.format]}</p>
            </div>

            <p className="font-display text-2xl text-cente ">
                {gig.venue}
